...
| HTTP 状态 | 总体状态 | 含义 |
|-------------|---------------|---------|
| `200` | `HEALTHY` | 用户可见平台可用。所有检查正常，或一个受控警告仍提供可用的最后一次正常数据 |
| `200` | `WARNING` | 至少一个警告影响可用性，或存在多个可操作警告，或一个受控警告超过所有探测键的 3% |
...
`summary` 字段：`total` 是每个探测键的一个条目，并随面板增加而增长 —— 请以你自己响应中的数值为准，而非本页示例。`warn` 是完整的可操作警告总数，但**不包括**按需为空的键；这些键在 `onDemandWarn` 中单独显示。`containedWarn` 是 `warn` 的子集，不是额外的计数桶。只有 `SEED_ERROR`、`COVERAGE_PARTIAL` 和 `COVERAGE_DEGRADED` 有资格受控，而且元数据必须证明记录数为正、不存在已知不可用情况，且不在有效宽限期内。只有一个可操作警告受到控制，且不超过所有探测键的 3% 时，可用性仍为 `HEALTHY`。两个或更多可操作警告始终影响可用性，包括来自同一播种器的多个检查。 受控判断会独立检查所有必需的诊断证据，不受最终状态选择顺序影响。证据必须与最终诊断状态一致，并且仍处于该检查现有的种子和内容新鲜度预算内。`STALE_SEED` 和 `STALE_CONTENT` 永不受控。可选的 `containmentUntil` 截止时间同时限制完整和精简判定缓存。制裁检查（`sanctionsPressure`、`sanctionsEntities`）、美国关税（`tariffTrendsUs`）和涉及再分发权限的供应链检查（`supplyVulnerability`、`supplyChokepointDependencies`）永不受控。陈旧年龄未知、读取策略或缓存状态不兼容、实物价格背离警告以及 `CHINA_DEGRADED` 均不受控；汇总行数不能证明底层中国数据集仍然可用。
