...
该保证的范围是有意限定的。位于 `/api/docs-mcp` 的独立[文档 MCP 服务器](/zh/agent-discovery)与位于 `/api/a2a` 的 A2A 端点同样会返回 `-32029`，但它们的限流器在解析请求体**之前**就已运行，因此手上没有 id，仍然回复 `"id": null`。对接这两个端点时，请对 null id 做防御性处理。
