...
- **YouTube 频道直播检测已停用**（#8167；#5503）。该检测通过住宅代理抓取 youtube.com 频道页面，#5503 指出这违反 YouTube 服务条款。在 `GET /api/aviation/v1/get-youtube-live-stream-info` 上，只传频道的查询现返回错误 `channel_live_detection_retired`，且不会请求 YouTube；`isLive` 始终为 `false`，`hlsUrl` 始终为空。`channel` 查询字段以及 `isLive` 与 `hlsUrl` 响应字段已弃用，为保持 v1 线格式兼容而保留；该操作本身未弃用。`GET /api/youtube/live?channel=` 现返回 HTTP 410 与 `{"error":"channel_live_detection_retired"}`，可缓存一天；`?videoId=` 仍返回该视频的 oEmbed 标题与频道名称。**迁移：**传入 `video_id`（在 `/api/youtube/live` 上为 `videoId`）以获取视频名称。频道直播检测没有替代方案；实时新闻与实时摄像头播放 `src/config/live-video-sources.ts` 中列出的已验证直播流。
