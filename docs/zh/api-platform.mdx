...
`summary.warn` 是完整的可操作警告总数。`summary.containedWarn` 是其子集，不是额外的计数桶。只有 `SEED_ERROR`、`COVERAGE_PARTIAL` 和 `COVERAGE_DEGRADED` 有资格受控，而且元数据必须证明记录数为正、不存在已知不可用情况，且不在有效宽限期内。只有一个可操作警告受到控制，且不超过所有探测键的 3% 时，可用性保持 `HEALTHY`；缺失或不可用的数据、未知状态、`REDIS_PARTIAL`、`ROLLOUT_PENDING`、更广泛的影响和严重故障不适用此规则。 受控判断会独立检查所有必需的诊断证据，不受最终状态选择顺序影响。证据必须与最终诊断状态一致，并且仍处于该检查现有的种子和内容新鲜度预算内。`STALE_SEED` 和 `STALE_CONTENT` 永不受控。可选的 `containmentUntil` 截止时间同时限制完整和精简判定缓存。制裁检查（`sanctionsPressure`、`sanctionsEntities`）、美国关税（`tariffTrendsUs`）和涉及再分发权限的供应链检查（`supplyVulnerability`、`supplyChokepointDependencies`）永不受控。陈旧年龄未知、读取策略或缓存状态不兼容、实物价格背离警告以及 `CHINA_DEGRADED` 均不受控；汇总行数不能证明底层中国数据集仍然可用。
