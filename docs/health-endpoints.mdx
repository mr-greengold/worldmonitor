...
|-------------|---------------|---------|
| `200` | `HEALTHY` | The user-visible platform is available. Checks are OK, or one contained warning still serves usable last-good data |
| `200` | `WARNING` | One or more warnings affect availability, or there are multiple actionable warnings or a contained warning exceeds 3% of all probed keys |
...
`summary.total` counts probed keys. `warn` is the complete actionable warning census and excludes on-demand empty keys, which appear in `onDemandWarn`. `containedWarn` is a subset of `warn`, not an additional bucket. The containment-eligible statuses are `SEED_ERROR`, `COVERAGE_PARTIAL`, and `COVERAGE_DEGRADED`. A warning is contained only when metadata proves a positive record count, no known unusability exists, and no pending grace is active. Availability remains `HEALTHY` only with one contained actionable warning that represents at most 3% of probed keys. Two or more actionable warnings always affect availability, including multiple checks from one seeder.

Containment checks all required diagnostics independently of the selected status. Evidence must still match the final verdict and remain within the check's existing seed and content freshness budgets. `STALE_SEED` and `STALE_CONTENT` are never contained. The optional `containmentUntil` deadline caps both full and compact cached verdicts. Sanctions checks (`sanctionsPressure`, `sanctionsEntities`), US tariffs (`tariffTrendsUs`), and redistribution-sensitive supply checks (`supplyVulnerability`, `supplyChokepointDependencies`) are never contained. Unknown freshness, incompatible reader policies or cache state, physical-divergence warnings, and `CHINA_DEGRADED` remain uncontained. Summary row counts do not prove that the underlying China datasets remain usable. Missing or unusable data, unknown warning states, `REDIS_PARTIAL`, `ROLLOUT_PENDING`, multiple actionable warnings or a contained warning above 3%, and every critical failure remain availability-affecting. `EMPTY_ON_DEMAND` and pending diagnostics retain their existing semantics. `staleContent` counts all `STALE_CONTENT` diagnostics, including entries inside their finite three-hour grace period. Active grace entries count in `ok` and the optional `pending` sub-count, not `warn`. `rolloutPending` is a subset of `warn`. Only `crit` drives `DEGRADED` or `UNHEALTHY`.
