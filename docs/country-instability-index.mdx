...
- **Stable**: score movement is between -1 and +1 points.
- **Falling**: score movement is less than -1 point.
- **Unspecified**: no valid prior snapshot is available, so no movement was
  measured.
...
24-hour prior snapshot; `trend` is then `TREND_DIRECTION_UNSPECIFIED`, which
separates a missing comparison from a measured zero (`TREND_DIRECTION_STABLE`).
