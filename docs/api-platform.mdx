...
`summary.warn` is the complete actionable warning census. `summary.containedWarn` is a subset, not an additional bucket. Only `SEED_ERROR`, `COVERAGE_PARTIAL`, and `COVERAGE_DEGRADED` are eligible for containment, and only with metadata-proven positive records, no known unusability, and no active pending grace. Containment checks all required diagnostics independently of the selected status. Evidence must still match the final verdict and remain within the check's existing seed and content freshness budgets. `STALE_SEED` and `STALE_CONTENT` are never contained. The optional `containmentUntil` deadline caps both full and compact cached verdicts. Sanctions checks (`sanctionsPressure`, `sanctionsEntities`), US tariffs (`tariffTrendsUs`), and redistribution-sensitive supply checks (`supplyVulnerability`, `supplyChokepointDependencies`) are never contained. Unknown freshness, incompatible reader policies or cache state, physical-divergence warnings, and `CHINA_DEGRADED` remain uncontained. Summary row counts do not prove that the underlying China datasets remain usable. Only one contained actionable warning, representing at most 3% of probed keys, keeps availability `HEALTHY`; missing or unusable data, unknown states, `REDIS_PARTIAL`, `ROLLOUT_PENDING`, broader impact, and critical failures do not.
