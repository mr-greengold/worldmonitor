...
| YouTube video metadata | `GET /api/aviation/v1/get-youtube-live-stream-info` | Names a YouTube video through oEmbed. Channel live detection is retired: a channel-only query returns error `channel_live_detection_retired`, and the `channel`, `isLive` and `hlsUrl` fields are deprecated. Not part of the six-tab auto-refresh loop. |
